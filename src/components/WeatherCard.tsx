...
export default function WeatherCard({ city, handelDeltCity }: Props) {
    ...
    return (
        <div className="border border-gray-300 rounded-md p-4 mt-4 bg-[#080d1766]" style={{
            backgroundImage: weather ? weatherGradient(weather.current.weather_code, !!weather.current.is_day) : undefined,
        }} >
            <div className="flex items-center justify-between">
                <div>
                    <span className="text-gray-400 text-sm font-medium flex items-center gap-1"> <MapPin className="h-3.5 w-3.5" /> {city.country}</span>
            ...
            <div className="flex items-center gap-4 mt-2 justify-between">
                ...
                <p className={weather?.current.is_day && weather?.current.weather_code === 0 ? "animate-spin-slow" : "animate-float"}>{weather ? <WeatherIcon code={weather.current.weather_code} isDay={!!weather.current.is_day} /> : "N/A"}{weather ? getWeatherCodeDesc[weather.current.weather_code] ?? "Unknown" : "N/A"}</p>
            </div>
            <div>
